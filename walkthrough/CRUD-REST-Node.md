...
[CRUD](https://en.wikipedia.org/wiki/Create,_read,_update_and_delete) is an acronym for Create-Read-Update-Delete. These are the four basic database functions. Realistically, however, most systems have a few more operations such as search or find. And read can refer to reading an individual object/record or reading a set of them.

[REST](https://en.wikipedia.org/wiki/REST) is an acronym for REpresentational State Transfer. It is an API design philosophy where a representation of an object is transferred from server to client, manipulated or updated on the client, and then sent back to the server. Originally, object representations were typically coded in [XML](https://en.wikipedia.org/wiki/XML) but today [JSON](https://en.wikipedia.org/wiki/JSON) encoding is more common and that's what we will use in this class. APIs designed according to REST principles are called RESTful APIs.
...
<table>
<tr><th>Operation</th><th>HTTP Verb></th><th>URL</th><th>Payload</th></tr>
<tr><td>Login</td><td>POST</td><td>https://restspace.dicax.org/api/login</td><td>{"username": "myusername", "password": "mypassword"}</td></tr>
<tr><td>Create</td><td>POST</td><td>https://restspace.dicax.org/api/items</td><td>{"make": "Pontiac", "model": "Firebird", "year": "1995"}</td></tr>
<tr><td>Read All</td><td>GET</td><td>https://restspace.dicax.org/api/items</td><td>(None)</td></tr>
<tr><td>Read By Id</td><td>GET</td><td>https://restspace.dicax.org/api/items/1</td><td>(None)</td></tr>
<tr><td>Find</td><td>GET</td><td>https://restspace.dicax.org/api/items?make=Pontiac</td><td>(None)</td></tr>
<tr><td>Update</td><td>PUT</td><td>https://restspace.dicax.org/api/items/1</td><td>{"make": "AMC", "model": "Gremlin", "year": "1976"}</td></tr>
<tr><td>Delete</td><td>DELETE</td><td>https://restspace.dicax.org/api/items/1</td><td>(none)</td></tr>
</table>
...
> This API will accept any username so long as the password is the same as the username but with "-9455" tacked onto the end. This is, of course, "Security by Obscurity" which isn't very secure at all because once someone figures out the secret they can make up other accounts. Nevertheless, it's handy for demo and walkthrough purposes and there are other safeguards against abuse that will be described later.
...
This initiates the request to the server. `fetch()` returns a promise. The `.then()` methods which follow indicate what should be done upon successful or failed calls.
...
The call to `Response.json()` returns another `Promise` so we have to use another `.then()` clause to manage the asynchronous function. Here, we report the entire object to the console (using `console.log()`) and then store the authorization token for future use.
...
If you get a 404 error you need to change the ID from `1` to the ID of something that's actually in the database.

The only difference between this and `ReadAll` is that the `id` argument is added to the end of the URL path.
...
Similarly, `Find` only differs from `ReadAll` by adding a query string. If user input were being used, you would need to call `encodeURIComponent()` on the values so that special characters don't cause problems.
...
Before leaving this walkthrough, experiment a bit. You'll find that `RestSpace` will store more than just cars. Try automating the creation of ten cars at a time and other interesting stuff. But please don't overload the database. It has a fixed capacity of 1000 items shared between all concurrent users. Once you exceed that, it will purge older items.

### Enhancements (left to the reader)

Session tokens expire after 90 minutes. But every request with a valid token will have an `Authentication-Info` response header containing an updated token refreshed for another 90 minutes. A more advanced caller would update the `authToken` global variable with that value.
...
It's a well-known principle that any online service that stores data will be misused. As noted before, the `RestSpace` service uses a password that's derived from the username. That's deliberately weak to enable experiential learning. To prevent abuse, the service has some deliberate weaknesses:

1. Data in the database is tied to a login *session*, not to a particular *username*. If two people log in using the same username, they will still see different sessions with different sets of items.
2. Data is only stored in memory and this is a serverless application. After a certain period of idleness, all data goes away and it starts fresh.
...
With these weaknesses, the service is only useful for practice.